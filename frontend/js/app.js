...
async function loadInfo() {
  ...
  const price = web3.utils.fromWei((info.deploymentConfig.tokensPerMint) , 'ether');
  //price = price * 0.0000000000000000001;
...
function setTotalPrice() {
  ...
  const totalPriceWei = BigInt(info.deploymentConfig.tokensPerMint) * BigInt(mintInputValue);
...
async function mint() {
  ...
  const value = BigInt(info.deploymentConfig.tokensPerMint) * BigInt(amount);
  const publicMintActive = await contract.methods.mintingActive().call();
  const presaleMintActive = await contract.methods.presaleActive().call();
